...
- 赛题生成器 v1.1.0 起按难度输出差异化要求段（入门=脚手架 / 进阶=四段报告纪律 /
  挑战=攻击用例+独立第二实现交叉核验），但底稿仍为三场景模板，须人工精修后
  发布——PROBLEMS.md 中标注了每题的来源与审核状态（全部"待审"）。
- 提交门户 v1.1.0 起必交项与 SUBMISSION.md 严格对齐（`.uibc` 包 / seal.json /
  AIGC 声明 / RUN.md），检测到可执行二进制直接判退。
- 评审与反作弊输出均带范围限定（scoped）口径：分数/风险级别仅反映规则加权，
  不构成整体背书；最终裁定权归组委会，不可让渡。
